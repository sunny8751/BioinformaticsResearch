...
class svm_train {
	...
	private void do_cross_validation() {
		...
		if (param.svm_type == svm_parameter.EPSILON_SVR
			...
			*/
			//add this R^2 value
				GridSearch.add(((prob.l * sumvy - sumv * sumy) * (prob.l * sumvy - sumv
										* sumy))
								/ ((prob.l * sumvv - sumv * sumv) * (prob.l * sumyy - sumy
										* sumy)));
		} else {
	...
	public static void main(String argv[]) throws IOException {
		...
		t.run(argv);
		//predict the testing data with the created model
		//a.txt is just a placeholder and is rewritten every time... the output file is not needed
		//svm_predict.main(new String[]{"-o", "scaled-converted-selected-"+Main.test, argv[4]+".model", "a.txt"});
		//svm_predict.main(new String[]{"scaled-converted-selected-"+Main.test, "scaled-converted-selected-"+Main.train".model", "a.txt"});

		if(!Main.finalModel){
		//if not the final thing
			//continue the GridSearch
			GridSearch.check();
		}
	}
